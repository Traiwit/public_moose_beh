...
* Raising a recoverable exception will cause this exception to be raised
  ...
  in the server function looks something like this:
   ...
   >>>     pipeException
   >>>       ("ValueError",
   >>>        "getFieldProps: Field %s does not exist in step %s frame %d.",
   >>>        fieldName.c_str() , currentStep->cStr(), currentFrame);
   >>>     return 1;
   >>>   }
 ...
 * input arguments: odbStep (string), odbFrame (int)
 *
...
int setFrame() {
  ...
  std::string odbStepNew = pipeReadString();
  int odbFrameNew = pipeReadInt();

  /* update step */
  if (odbStepNew != currentStep->cStr()) {
    if (ptOdb->steps().isMember(odbStepNew.c_str())) {
      debug_print("setFrame: changing from step %s to step %s...\n",
                  currentStep->cStr(), odbStepNew.c_str());
      *(currentStep) = odbStepNew.c_str();
      delete ptOdbStep;
      ptOdbStep = new odb_Step(ptOdb->steps()[currentStep->cStr()]);
      currentFrame = -1;
      delete ptOdbFrame; ptOdbFrame = NULL;
    }
    else {
      /* new step not there: don't change step and frame */
      pipeException
        ("ValueError",
         "setFrame: There is no step %s in the odb.", odbStepNew.c_str());
      return 1;
    }
  }
  ...
  if (ptOdbStep) {
    ...
    if (framesCnt < 1) {
      ...
      pipeException
        ("ValueError",
         "setFrame: There is no frame in step %s.", odbStepNew.c_str());
      return 1;
    }

    /* negative frame numbers count from the end */
    while (odbFrameNew < 0) {
      odbFrameNew += framesCnt;
    }
  }

  if (ptOdbStep && framesCnt > 0 && odbFrameNew != currentFrame) {
    debug_print("setFrame: changing from frame %d to frame %d...\n",
        	currentFrame, odbFrameNew);

    /* check that frequested frame actually exists */
    if (odbFrameNew >= framesCnt) {
      currentFrame = -1;
      delete ptOdbFrame; ptOdbFrame = NULL;
      pipeException
        ("ValueError",
         "setFrame: There is no frame %d in step %s, last frame is"
         " %d.", odbFrameNew, currentStep->cStr(), framesCnt-1);
      return 1;
    }
    else if (odbFrameNew != currentFrame) {
      currentFrame = odbFrameNew;
      delete ptOdbFrame;
      ptOdbFrame = new odb_Frame(ptOdbStep->frames(odbFrameNew));
    }
  }
...
/* getOdbStepNames
 *
 * read odb step names from the odb
 * input argument: ---
 *
 * output arguments: number of odb steps (int),
 *                   step name 1 (string), step name 2, ...
 */
int getOdbStepNames() {
  debug_print("This is getOdbStepNames...\n");

  /* report number of steps */
  int nbOfSteps = ptOdb->steps().size();
  pipeWrite( nbOfSteps );

  /* iterate over odb steps and report names */
  odb_StepRepositoryIT stepIter( ptOdb->steps() );
  int i = 0;
  for (stepIter.first();
       !stepIter.isDone() && i<nbOfSteps;
       stepIter.next(), ++i) {
    pipeWrite( stepIter.currentKey().cStr() );
  }

  /* check number agains iteration */
  if (i != nbOfSteps) {
    pipeException
      ("RuntimeError",
       "getOdbStepNames: Inconsistent number of steps in the odb:"
       " Steps.size=%d; nb of steps through iteration: %d",
       nbOfSteps, i);
    return 1;
  }
  return 0;
...
/* getOdbFrames
 *
 * get the number of output frames for the specified odb step
 * input argument: odb step name
 *
 * output argument: number of odb frames (int)
 */
int getOdbFrames() {

  debug_print("This is getOdbFrames...\n");

  /* get odbStep argument */
  std::string odbStep = pipeReadString();

  /* check if step exists in the odb */
  if (!ptOdb->steps().isMember(odbStep.c_str())) {
    /* requested step not there */
    pipeException
      ("ValueError",
       "getOdbFrames: There is no step %s in the odb.", odbStep.c_str());
    return 1;
  }

  /* determine and report number of frames */
  debug_print("This is getOdbFrames, querying odb step %s\n", odbStep.c_str());
  int framesCnt;
  framesCnt = ptOdb->steps()[odbStep.c_str()].frames().size();
  pipeWrite( framesCnt );
...
void initFuncMap() {
  ...
  funcMap["getOdbStepNames"] = getOdbStepNames;
  funcMap["getOdbFrames"] = getOdbFrames;
  // funcMap[""] = ;
  // funcMap[""] = ;
  // funcMap[""] = ;
  // funcMap[""] = ;

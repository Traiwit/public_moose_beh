...
def transformEigen(S, V,
    ...
    @param eigenFmt: format of eigenvectors:
...
###GP: I think this interface needs a cleanup...
###GP: maybe have a look at bae.misc_01.KDTree ?
###GP: and I suggest to loot the docs
class KDTree(object):
    ...
    @staticmethod
    def uniqueTreeIds(tree, atol=1E-6, normType='sphere'):
        ...
        if normType == 'sphere':
            ...
            raise ValueError("Don't know normType %s" % str(normType))
    ...
    def ensureUnique(cls, arg, atol=1E-6):
        """ checks if a PointCloud (or its KDTree) has only unique Points
        within a tolerance
        """
        if not cKDTree:
            msg('ERROR. It seems that scipy is not installed properly.')
        ...
        idx = cls.uniqueTreeIds(tree, atol=atol)

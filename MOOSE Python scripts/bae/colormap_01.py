"""create colormaps
used with rhino so far but can be used for everything execpt making coffee
(use emacs for that).
...
======
 ...
 1. using predefined colormaps: (see which are already defined at the end)

  >>> from colormap_01 import colormap_rainbow as colormap
  >>> colormap.set_xrange(xmin, xmax)
  >>> r,g,b = colormap.get_rgb_color(x)


 2. creating a new colormap:

  >>> from colormap_01 import Colormap
  >>> colormap = Colormap(
  >>>     x = (  0,   1,   2,   3,   4),
  >>>     r = (  0,   0,   0, 255, 255),
  >>>     g = (  0, 255, 255, 255,   0),
  ...
  Define colorsteps between which the r, g, b values are interpolated linearily
  in this example::
  ...
  use the x-spacing to stretch or compress certain regions. E.g. to see more
  orange (between yellow and red) change the above table to::

     2 | green
     3 | yellow
     5 | red

  Note that the x-values do not correspond to the actual x-values used when
  calling get_rgb_color(x): The x range specified by set_xrange() is linearily
  mapped on the x range specified in the constructor Colormap(). The x range
  specified in the constructor Colormap() may by anything, e.g. degrees on a
  color circle as is done with the first predefined colormaps

  after declaring the colormap, use it as before:
...
bugs/limits:
============
 - Discrete colormaps should possibly be dealt with by a simpler seperate
   implementation of discrete colormaps where the distinct colors are explicitly
...
_version_history_ = """\
1.0 GP newly extracted from make_rhino_spheres
1.1 GP added flexibility to go without numpy
1.2 TR added ALL ColorBrewer schemes and their numpy-free interpolation
"""
...
class Colormap(object):
    """interpolate colors
    Constructor-arguments are sequences (arrays, lists, ...) :
    x: value interpolation point
    r, g, b: appropriate r,g,b-colour-values (in the range of 0 to 255)
    """
    def _winp_init(self, x, r, g, b, num_colors = None):
        "Constructor: numpy version"
        self.x = np.array(x, dtype = float)
        self.r = np.array(r, dtype = float)
        self.g = np.array(g, dtype = float)
        self.b = np.array(b, dtype = float)
        assert(self.x.shape == self.r.shape)
        assert(self.x.shape == self.g.shape)
        assert(self.x.shape == self.b.shape)

        self.num_colors = num_colors
    def _wonp_init(self, x, r, g, b, num_colors = None):
        "Constructor: version without numpy"
        self.x = x[:]
        self.rgb = zip(r,g,b)
        assert(len(x) == len(r))
        assert(len(x) == len(g))
        ...
        self.num_colors = num_colors
    __init__ = selectattrversion(np, _winp_init, _wonp_init, """
        Constructor

        @param x: value interpolation point in ascending order!
        @param r, g, b: corresponding r,g,b-colour-values (in the range of 0
           to 255)
        """)

    # works for both with and without numpy
    def set_xrange(self, xmin, xmax):
        """
        calculates the constants for scaling of x values

        let x_val in [ xmin, xmax ]
        and x_tab in [ self.x[0], self.x[-1] ]
        then: x_tab := sc_a*x_val + sc_b
        """
        xmin, xmax = map(float, [xmin, xmax])
        self.sc_a = (self.x[-1]-self.x[0])/(xmax-xmin)
        self.sc_b = (self.x[-1]*xmin-self.x[0]*xmax)/(xmin-xmax)

        if self.num_colors:
            num_col = self.num_colors+0.999
            self.sc_d = num_col/(xmax-xmin)
            self.sc_e = num_col*xmin/(xmin-xmax)


    def _winp_get_rgb_color(self, x):
        if self.num_colors:
            xi = int(self.sc_d*x + self.sc_e)
            # print xi
            xx = (self.x[-1]-self.x[0])*(float(xi)/self.num_colors) + self.x[0]
        else:
            xx = self.sc_a*x + self.sc_b

        rgb = (int(np.interp(xx,self.x,self.r)),
               int(np.interp(xx,self.x,self.g)),
               int(np.interp(xx,self.x,self.b)))
        return rgb
    def _wonp_get_rgb_color(self, x):
        if self.num_colors:
            xi = int(self.sc_d*x + self.sc_e)
            xx = (self.x[-1]-self.x[0])*(float(xi)/self.num_colors) + self.x[0]
        else:
        ...
        if xx<=self.x[0]:
            ...
            index = [xx<=xtab for xtab in self.x].index(True)
            xi = (xx-self.x[index-1])/(self.x[index]-self.x[index-1])
            rgb = [c0*(1.0-xi)+c1*xi
                   for c0, c1 in izip(self.rgb[index-1], self.rgb[index])]
        return rgb
    get_rgb_color = selectattrversion(
        np, _winp_get_rgb_color, _wonp_get_rgb_color, """
        return the rgb tuple (three integer in the range of 0..255) for the
        given x value.
        """)
...
colormap_rainbow = Colormap(
    x = (0,    30,  90, 150, 210, 270, 300),
    r = (133,   0,   0,   0, 255, 255, 255),
    g = (  0,   0, 255, 255, 255,   0,   0),
    b = (255, 255, 255,   0,   0,   0, 165))

colormap_abaqus = Colormap(
    x = ( 30,  90, 150, 210, 270),
    r = (  0,   0,   0, 255, 255),
    g = (  0, 255, 255, 255,   0),
    b = (255, 255,   0,   0,   0))

colormap_rainbow_d8 = Colormap(
    x = (0,    30,  90, 150, 210, 270, 300),
    r = (133,   0,   0,   0, 255, 255, 255),
    g = (  0,   0, 255, 255, 255,   0,   0),
    b = (255, 255, 255,   0,   0,   0, 165),
...
colormap_opti_d = Colormap(
    x = (0,    30, 100, 150, 160, 180, 190, 210, 300),
    r = (133,   0,   0,   0, 250, 255, 255, 255, 255),
    g = (  0,   0, 255, 255, 255, 255, 250,   0,   0),
    b = (255, 255, 255,   0,   0,   0,   0,   0, 165),
    num_colors = 7)

################################################################################
### colorBrewer2 ColorSchmes -- see www.colorbrewer2.org                     ###
### Schemes are only available as hard coded lists                           ###
### RGB-Definitions taken from:                                              ###
...
sequential = {
    'multihue' : {'YlGn': YlGn, 'YlGnBu': YlGnBu, 'GnBu': GnBu, 'BuGn': BuGn,
                  'PuBuGn': PuBuGn, 'PuBu': PuBu, 'BuPu': BuPu, 'RdPu': RdPu,
                  'PuRd': PuRd, 'OrRd': OrRd, 'YlOrRd': YlOrRd, 'YlOrBr': YlOrBr },
    'singlehue': {'Purples': Purples, 'Blues': Blues, 'Greens': Greens,
                  'Oranges': Oranges, 'Reds': Reds, 'Greys': Greys },
}
...
diverging = {'PuOr': PuOr, 'BrBG': BrBG, 'PRGn': PRGn, 'PiYG': PiYG,
             'RdBu': RdBu, 'RdGy': RdGy, 'RdYlBu': RdYlBu,
             'Spectral': Spectral, 'RdYlGn': RdYlGn }
...
qualitative = {'Pastel2':Pastel2, 'Pastel1':Pastel1, 'Dark2':Dark2,
               'Accent':Accent, 'Paired':Paired, 'Set1':Set1,
               'Set2':Set2, 'Set3':Set3 }


class ColorSet(object):
    """
    Defines a ColorScheme by rgb-tuples and provides its (linear) interpolation
    """
    def __init__(self,colorDict,name='notNamed',description=''):
        """
        @param colorDict: dictionary holding lists of rgb-tuples for colormap;
        key equals the length of each lists
        @param name: name of colormap
        @param description: descriptionstring for colormap
        """
        self.colorDict     = colorDict
        self.NmaxNoInterP  = sorted(colorDict.keys())[-1]
        self.NminNoInterP  = sorted(colorDict.keys())[0]
        self.name          = name
        self.description   = description
        self.reverse       = False
        self.forceInterpol = False

    def _colorInterpol(self,N):
        """
        Linear interpolation of RGB-Values for a number of N colorbands.
        @param N: number of colorbands
        @return: list if interpolated rgb-tuples
        """
        ipBase = self.colorDict[self.NmaxNoInterP]
        import bisect
        cmap = []
        Ns = (self.NmaxNoInterP-1)/float(N)
        for step in range(N):            
            iL = int(step*Ns)
            iU = iL+1
            cmap.append(tuple( [
                int( (ipBase[iU][i]-ipBase[iL][i])*(step*Ns-iL)+ipBase[iL][i] )
                                 for i in range(3)
                               ]   ) )
        return map(tuple,cmap)

    def _colorList(self,N):
        """ prepare colorList for output
        """
        if self.reverse:
            direct = -1
        else:
        ...
        if (N > self.NmaxNoInterP) or \
           (N < self.NminNoInterP) or \
           self.forceInterpol:
            cmap = self._colorInterpol(N)
        else:
            cmap = self.colorDict[N]

        return cmap[::direct]

    def asRGBStr(self,N,prefix='rgb'):
        """
        Return colormap for N-colorbands as a string list of rgb-tuples
        @param N: number of colorbands
        @param prefix: string prefix for each color string
        @return: list if (interpolated) rgb-tuples as strings
        """
        cmap = self._colorList(N)
        return [prefix+"(%d,%d,%d)" % col for col in cmap]

    def asHexStr(self,N,prefix=''):
        """
        Return colormap for N-colorbands as a string list of hex-values
        @param N: number of colorbands
        @param prefix: string prefix for each color string
        @return: list if (interpolated) hex-values as strings
        """
        cmap = self._colorList(N)
        return [prefix + r"#%02x%02x%02x" % col for col in cmap]

    def rgb(self,N):
        """
        Return colormap for N-colorbands as a list of rgb-tuples
        @param N: number of colorbands
        @return: list if (interpolated) rgb-tuples
        """
        return self._colorList(N)

    def toColormap(self,N):
        """ returns a Colormap object
        @param N: number of colorbands
        """
        r,g,b = [],[],[]
        for col in self.rgb(N):
            r.append(col[0])
            g.append(col[1])
            b.append(col[2])
        return Colormap(x=range(N),r=r,g=g,b=b,num_colors=N)


from bae.misc_01 import Container
class ColorBrewer(object):
    """
    Provides the colorBrewer2 color schemes (see U{http://colorbrewer2.org/} for
    more details).  Dependend on the nature of data three different groups of
    schemes are provided:
        1. sequential
            - Single hue: I{Purples, Blues, Greens, Oranges,  Reds, Greys}
            - Multi hue: I{YlGn, YlGnBu, GnBu, BuGn, PuBuGn, PuBu, BuPu, RdPu,
            PuRd, OrRd, YlOrRd, YlOrBr}

        2. qulitative: I{Pastel2, Pastel1, Dark2,  Accent, Paired, Set1, Set2,
        Set3}

        3. diverging: I{PuOr, BrBG, PRGn, PiYG, RdBu, RdGy, RdYlBu, Spectral,
        RdYlGn}

    They can be accessed directly by name or by group and name.

    Example:
    ========
    >>> from bae.colormap_01 import ColorBrewer
    >>> cB = ColorBrewer()
    >>> print cB.Reds.asHexStr(15)                      #hexOutput by Name
    >>> print cB.sequentialSingleHue.Red.asRGBStr(15)   #rgbOutput by group.Name
    >>> reds = cB.Reds                                  #get colorscheme
    >>> reds.forceInterpol = True                       #force Interpolation
    >>> reds.reverse = True                             #reverse order
    >>> print reds.rgb(5)                               #rgb-tuple
    >>> redsCM = reds.toColormap(12)                    #create Colormap object

    """
    def __init__(self):

        for mapsType, maps in [
                ('sequentialSingleHue', sequential['singlehue']),
                ('sequentialMultiHue', sequential['multihue']),
                ('qualitative', qualitative),
                ('diverging', diverging),]:
            dd = dict(
                (name, ColorSet(cmap,name,mapsType))
                for name,cmap in maps.iteritems())
            setattr(self,mapsType,Container(**dd))
            for cmapName,cmap in maps.iteritems():
                setattr(self,cmapName,ColorSet(cmap,cmapName,mapsType))

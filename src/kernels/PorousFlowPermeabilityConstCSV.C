...
PorousFlowPermeabilityConstCSV::validParams()
{
  ...
  params.addParam<MaterialPropertyName>(
      "excav_perm", 0, "The coupled variable which provides the force");
  return params;
}
...
PorousFlowPermeabilityConstCSV::computeQpProperties()
{
  ...
  {
    _perm[_qp](0) =
        std::min(exp(_damage[_qp] * _aw[_qp](0)) * _mat_prop[_qp](0) * 1e-7, _kmax[_qp](0) * 1e-7);
    _perm[_qp](1) =
        std::min(exp(_damage[_qp] * _aw[_qp](0)) * _mat_prop[_qp](1) * 1e-7, _kmax[_qp](0) * 1e-7);
    _perm[_qp](2) =
        std::min(exp(_damage[_qp] * _aw[_qp](0)) * _mat_prop[_qp](2) * 1e-7, _kmax[_qp](0) * 1e-7);
  }
  ...
  RealTensorValue permeability(_perm[_qp](0), 0, 0, 0, _perm[_qp](1), 0, 0, 0, _perm[_qp](2));

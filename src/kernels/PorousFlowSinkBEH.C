    ...
    _perm_derivs(_dictator.usePermDerivs())
{
  ...
  if (_use_mass_fraction && !_has_mass_fraction)
    mooseError("PorousFlowSinkBEH: You have used the use_mass_fraction flag, but you have no "
               "mass_fraction Material");

  if (_use_mobility && !_has_mobility)
    mooseError("PorousFlowSinkBEH: You have used the use_mobility flag, but there are not the "
...
Real
PorousFlowSinkBEH::computeQpResidual()
{
  ...
  if (_use_mobility)
  {
    ...
    flux *= (*_fluid_density_node)[_i][_ph] * k / ((_v[_qp]) * (*_fluid_viscosity)[_i][_ph]);
  }
  if (_use_relperm)
...
Real
PorousFlowSinkBEH::computeQpJacobian()
{
  return jac(_var.number());
}

Real
PorousFlowSinkBEH::computeQpOffDiagJacobian(unsigned int jvar)
{
  return jac(jvar);
}

Real
PorousFlowSinkBEH::jac(unsigned int jvar) const
{
...
Real
PorousFlowSinkBEH::multiplier() const
{
  return _m_func.value(_t, _q_point[_qp]);
}

Real
PorousFlowSinkBEH::dmultiplier_dvar(unsigned int /*pvar*/) const
{
  return 0.0;
}
